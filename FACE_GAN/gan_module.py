...
class AgingGAN(pl.LightningModule):
    ...
    def load_from_checkpoint(self, checkpoint_path):
        youngToOld = checkpoint_path + 'YoungToOld.pth'
        oldToYoung = checkpoint_path + 'OldToYoung.pth'
        self.genA2B.load_state_dict(torch.load(youngToOld))
        self.genB2A.load_state_dict(torch.load(oldToYoung))
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            loss_identity_B = F.mse_loss(same_B, real_B) * self.hparams['identity_weight']
            # G_B2A(A) should equal A if real A is fed
            same_A = self.genB2A(real_A)
            loss_identity_A = F.mse_loss(same_A, real_A) * self.hparams['identity_weight']

            # GAN loss
            fake_B = self.genA2B(real_A)
            pred_fake = self.disGB(fake_B)
            loss_GAN_A2B = F.binary_cross_entropy(pred_fake, torch.ones(pred_fake.shape).type_as(pred_fake)) * self.hparams[
                'adv_weight']

            fake_A = self.genB2A(real_B)
            pred_fake = self.disGA(fake_A)
            loss_GAN_B2A = F.binary_cross_entropy(pred_fake, torch.ones(pred_fake.shape).type_as(pred_fake)) * self.hparams[
                'adv_weight']

            # Cycle loss
            recovered_A = self.genB2A(fake_B)
            loss_cycle_ABA = F.mse_loss(recovered_A, real_A) * self.hparams['cycle_weight']

            recovered_B = self.genA2B(fake_A)
            loss_cycle_BAB = F.mse_loss(recovered_B, real_B) * self.hparams['cycle_weight']
        ...
        if optimizer_idx == 1:
            ...
            loss_D_real = F.binary_cross_entropy(pred_real, torch.ones(pred_real.shape).type_as(pred_real))

            # Fake loss
            fake_A = self.generated_A
            pred_fake = self.disGA(fake_A.detach())
            loss_D_fake = F.binary_cross_entropy(pred_fake, torch.zeros(pred_fake.shape).type_as(pred_fake))
